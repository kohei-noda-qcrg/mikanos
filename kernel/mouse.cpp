...
namespace
{
    ...
    void EraseMouseCursor(PixelWriter *pixel_writer, Vector2D<int> postion, PixelColor erase_color)
    {
        for (int dy = 0; dy < kMouseCursorHeight; ++dy)
            for (int dx = 0; dx < kMouseCursorWidth; ++dx)
            {
                if (mouse_cursor_shape[dy][dx] != ' ')
                    pixel_writer->Write(postion.x + dx, postion.y + dy, erase_color);
            }
    }
}
...
{
    Log(kInfo, "MouseCursor::MoveRelative: displacement = (%d, %d)\n", displacement.x, displacement.y);
